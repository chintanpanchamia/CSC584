...
public class BoundarySteering
{
	...
	{
		PVector futureRay1 = PVector.add(character.position,
				PVector.mult(character.velocity.copy().setMag(FUTURE_RAY_VEL_BASE), 3f));
		PVector futureRay2 = PVector.add(character.position,
				PVector.fromAngle(character.velocity.heading() - PConstants.PI / 4f).setMag(FUTURE_RAY_VEL_BASE * 2f));
		PVector futureRay3 = PVector.add(character.position,
				PVector.fromAngle(character.velocity.heading() + PConstants.PI / 4f).setMag(FUTURE_RAY_VEL_BASE * 2f));

		PVector boundary = new PVector(parent.width, parent.height);
		return (checkRayWithBoundary(futureRay1, BORDER_PADDING, boundary)
	...
	{
		return (futureRay.x <= BORDER_PADDING || futureRay.x >= boundary.x - BORDER_PADDING
				|| futureRay.y <= BORDER_PADDING || futureRay.y >= boundary.y - BORDER_PADDING);
	}
	...
	public static float avoidBoundaryOnWander(Kinematic character, PApplet parent, int BORDER_PADDING)
	{
		...
		} while (checkForBoundaryAvoidance(
				new Kinematic(character.position, PVector.fromAngle(targetOrientation), 0, 0), parent, BORDER_PADDING));
